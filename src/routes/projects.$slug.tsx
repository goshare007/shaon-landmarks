...
export const Route = createFileRoute("/projects/$slug")({
	...
	pendingComponent: () => (
		<main>
			<section className="relative h-[921px] overflow-hidden bg-surface-container-high">
				<div className="mx-auto flex h-full max-w-[1440px] flex-col justify-end px-4 pb-24 md:px-16">
	...
	head: ({ loaderData }) => {
		...
		const description = project?.description ?? "Explore this premium Shaon Landmarks development.";
	...
	notFoundComponent: () => (
		<main className="flex min-h-[60vh] items-center justify-center bg-surface">
			<div className="text-center">
				...
				<Link
					to="/portfolio"
					className="mt-8 inline-block rounded-sm border border-outline-variant px-6 py-3 text-[11px] font-medium tracking-[0.1em] text-on-surface uppercase transition-colors hover:border-secondary hover:text-secondary"
				>
					Back to Portfolio
				</Link>
			</div>
...
function ProjectDetail() {
	...
	if (!project) return <Route.notFoundComponent />;
...
function SimpleProjectView({ project }: { project: Project }) {
	return (
		<main>
			<section className="relative h-[60vh] min-h-[24rem] overflow-hidden">
				<motion.div
			...
			<section className="bg-surface py-24">
				<div className="mx-auto max-w-[1440px] px-4 md:px-16">
					...
					<div className="mt-12">
						<Link
							to="/portfolio"
							className="inline-flex items-center gap-2 text-[11px] font-medium tracking-[0.1em] text-on-surface uppercase transition-colors hover:text-secondary"
						>
...
function HeroSection({ project }: { project: Project }) {
	const d = project.detail!;
	return (
		<section className="relative h-[921px] overflow-hidden">
			...
			/>
			<div className="absolute inset-0 z-20 mx-auto flex max-w-[1440px] flex-col justify-end px-4 pb-24 md:px-16">
				<motion.div
...
function SpecsSection({ specs }: { specs: Project["detail"]["specs"] }) {
	const items = [
...
function VisionSection({ vision }: { vision: Project["detail"]["vision"] }) {
	return (
		<section className="bg-surface py-32">
			<div className="mx-auto flex max-w-[1440px] flex-col gap-20 px-4 md:flex-row md:px-16">
				<motion.div
	...
	return (
		<section className="bg-surface-container-low py-24">
			<div className="mx-auto max-w-[1440px] px-4 md:px-16">
				...
				<div className="grid h-[800px] grid-cols-12 grid-rows-2 gap-6">
					<motion.div
...
function AmenitiesSection({
	...
	amenities: Project["detail"]["amenities"];
}) {
	return (
...
function LocationSection({
	...
	location: Project["detail"]["location"];
}) {
	return (
		<section className="bg-surface py-32">
			<div className="mx-auto max-w-[1440px] px-4 md:px-16">
				<div className="grid items-center gap-16 lg:grid-cols-12">
						...
						<div className="space-y-8">
							{location.points.map((point) => (
								...
								>
									...
									<div>
										<h4 className="mb-1 text-[11px] font-medium tracking-[0.1em] text-primary uppercase">
											{point.title}
										</h4>
					...
					<motion.div
						className="relative h-[600px] overflow-hidden bg-surface-container-highest lg:col-span-7"
						initial={{ opacity: 0, scale: 1.03 }}
						whileInView={{ opacity: 1, scale: 1 }}
						viewport={{ once: true, margin: "-80px" }}
