    ...
    if (linkMatch) {
      ...
      parts.push(
        <a
          key={n}
          href={linkMatch[2]}
          className='text-secondary underline underline-offset-2 transition-colors hover:opacity-80'
        >
...
export function renderMarkdown(content: string): ReactNode[] {
  ...
  while (i < lines.length) {
    ...
    if (line.match(/^[-*]\s/)) {
      ...
      while (i < lines.length && lines[i].match(/^[-*]\s/)) {
        ...
        items.push(
          <li
            key={n}
            className='relative pl-6 before:absolute before:left-0 before:top-[0.6em] before:h-1.5 before:w-1.5 before:rounded-full before:bg-secondary'
          >
    ...
    nodes.push(
      <p key={`p-${i}`} className='mb-5 leading-relaxed text-on-surface/85'>
