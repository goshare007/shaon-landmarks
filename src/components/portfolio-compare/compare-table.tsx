...
const labelClass =
  'text-label font-medium tracking-widest text-secondary uppercase';

function cellClass(highlight?: boolean) {
  return `p-4 text-sm leading-relaxed text-on-surface ${
      ...
      <table className='w-full min-w-[600px] border-collapse'>
        <thead>
          <tr className='border-b border-outline-variant'>
            <th className='sticky left-0 z-10 min-w-28 bg-surface p-4 text-left text-label font-medium tracking-widest text-secondary uppercase'>
              Feature
            </th>
            {projects.map((p) => (
              <th key={p.id} className='min-w-44 p-4 text-left'>
                ...
                >
                  ...
                  <h3 className='font-serif text-base text-primary transition-colors group-hover:text-secondary md:text-lg'>
                    {p.title}
                  </h3>
                </Link>
              </th>
        ...
        <tbody>
          ...
          />
          <tr className='border-b border-outline-variant'>
            <td className={`${labelClass} sticky left-0 z-10 bg-surface p-4`}>
              Key Highlights
            </td>
            {projects.map((p) => (
